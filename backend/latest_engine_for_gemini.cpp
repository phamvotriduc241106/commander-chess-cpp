...
struct MoveList {
    MoveTriple moves[MaxMoves];
    int count = 0;

    inline void push(const MoveTriple& m) {
        assert(count < (int)MaxMoves);
        moves[count++] = m;
    }
    inline void clear() { count = 0; }
    inline bool empty() const { return count == 0; }
    inline size_t size() const { return (size_t)count; }
    inline MoveTriple& operator[](size_t i) { return moves[i]; }
    inline const MoveTriple& operator[](size_t i) const { return moves[i]; }
    inline MoveTriple* begin() { return moves; }
    inline MoveTriple* end() { return moves + count; }
    inline const MoveTriple* begin() const { return moves; }
    inline const MoveTriple* end() const { return moves + count; }
};

using AllMoves = MoveList<256>;
...
static void get_moves_with_ctx(const Piece& piece, const MoveGenContext& ctx,
                               MoveList<256>& out_moves) {
    BB132 bb = get_move_mask_bitboard(piece, ctx);
    // Preserve legacy deterministic ordering: (col,row).
    for (int c = 0; c < COLS; c++) {
        for (int r = 0; r < ROWS; r++) {
            int sq = sq_index(c, r);
            if (bb.test(sq)) out_moves.push({piece.id, c, r});
        }
    }
}
...
static std::string check_win(const PieceList& pieces, const std::string& last, GameMode mode) {
...
static void all_moves_for(const PieceList& pieces, const std::string& player,
                          MoveList<256>& out_moves) {
    out_moves.clear();
    MoveGenContext ctx = build_movegen_context(pieces);
    for (auto& p : pieces) {
        if (p.player != player) continue;
        get_moves_with_ctx(p, ctx, out_moves);
    }
}
...
static bool has_immediate_winning_move(const PieceList& pieces, const std::string& player,
                                       GameMode mode) {
    MoveList<256> moves;
    all_moves_for(pieces, player, moves);
    for (auto& m : moves) {
        PieceList np = apply_move(pieces, m.pid, m.dc, m.dr, player);
        if (!check_win(np, player, mode).empty()) return true;
    }
    return false;
}
...
struct SearchState {
    static constexpr int MAX_TRACKED_PIECE_ID = 256;
    PieceList pieces;
    std::string turn;
    GameMode mode = GameMode::FULL_BATTLE;
    uint64_t hash = 0;
    ...
    // O(1) piece-id lookup for hot search paths.
    std::array<int, MAX_TRACKED_PIECE_ID> idx_by_id{};
    // Cached piece counts by side and kind-index.
    int kind_count[2][11]{};
    ...
    void rebuild_caches() {
        ...
        for (int i = 0; i < (int)pieces.size(); i++) {
            ...
            if (p.id >= 0 && p.id < MAX_TRACKED_PIECE_ID) idx_by_id[p.id] = i;
            int ki = kind_index(p.kind);
            if (ki >= 0 && ki < 11) kind_count[pi][ki]++;
...
static inline int find_piece_idx_by_id(const SearchState& st, int pid) {
    if (pid >= 0 && pid < SearchState::MAX_TRACKED_PIECE_ID) return st.idx_by_id[pid];
...
static bool validate_state_for_sim(const PieceList& pieces,
    ...
    if (terminal_ok && !check_win(pieces, last_mover, mode).empty()) return true;
...
static int see(const PieceList& pieces, int col, int row,
    ...
    for (auto& p : pieces) {
        ...
        MoveList<256> mvs;
        get_moves_with_ctx(p, ctx, mvs);
        for (auto& m : mvs) {
            if (m.dc == col && m.dr == row) {
...
static AllMoves order_moves(const AllMoves& moves, const PieceList& pieces,
    ...
    struct ScoredMove { int score; MoveTriple move; };
    ...
    int id_to_idx[SearchState::MAX_TRACKED_PIECE_ID];
    std::fill(std::begin(id_to_idx), std::end(id_to_idx), -1);
    for (int i = 0; i < (int)pieces.size(); i++) {
        int pid = pieces[i].id;
        if (pid >= 0 && pid < SearchState::MAX_TRACKED_PIECE_ID) id_to_idx[pid] = i;
    }
    for (auto& m : moves) {
        const Piece* piece = nullptr;
        if (m.pid >= 0 && m.pid < SearchState::MAX_TRACKED_PIECE_ID) {
            int idx = id_to_idx[m.pid];
            if (idx >= 0 && idx < (int)pieces.size()) piece = &pieces[idx];
        } else {
            for (const auto& p : pieces) if (p.id == m.pid) { piece = &p; break; }
        }
        if (!piece) continue;
        ...
        if (scored_count < 256) scored[scored_count++] = {score, m};
    }
    ...
    AllMoves result;
    for (int i = 0; i < scored_count; i++) result.push(scored[i].move);
    return result;
}
...
static int attackers_to_square(const PieceList& pieces, int col, int row,
    ...
    for (auto& p : pieces) {
        ...
        get_moves_with_ctx(p, ctx, mvs);
        for (auto& m : mvs) {
            if (m.dc == col && m.dr == row) { attackers++; break; }
        }
    }
    return attackers;
}
...
// Cheap context terms added on top of incremental quick_eval.
// Designed for pruning decisions: no expensive full legal-move generation.
static int quick_eval_context_terms(SearchState& st, const std::string& cpu_player) {
    build_attack_cache(st);
    int cpu_pl = (cpu_player == "red") ? 0 : 1;
    int opp_pl = 1 - cpu_pl;
    auto kc = [&](int pl, int ki) -> int { return st.kind_count[pl][ki]; };

    const int cpu_n  = kc(cpu_pl, 10), opp_n  = kc(opp_pl, 10);
    const int cpu_af = kc(cpu_pl, 9),  opp_af = kc(opp_pl, 9);
    const int cpu_in = kc(cpu_pl, 2),  opp_in = kc(opp_pl, 2);
    const int cpu_t  = kc(cpu_pl, 4),  opp_t  = kc(opp_pl, 4);
    const int cpu_a  = kc(cpu_pl, 6),  opp_a  = kc(opp_pl, 6);
    const int cpu_land = cpu_t + cpu_in + cpu_a;
    const int opp_land = opp_t + opp_in + opp_a;

    int score = 0;

    // Division health pressure (mode-aware objective shaping).
    switch (st.mode) {
    case GameMode::MARINE_BATTLE:
        score += (cpu_n - opp_n) * 620;
        break;
    case GameMode::AIR_BATTLE:
        score += (cpu_af - opp_af) * 520;
        break;
    case GameMode::LAND_BATTLE:
        score += (cpu_land - opp_land) * 340;
        score += (cpu_t - opp_t) * 90;
        score += (cpu_in - opp_in) * 55;
        score += (cpu_a - opp_a) * 80;
        break;
    case GameMode::FULL_BATTLE:
    default:
        score += (cpu_n - opp_n) * 90;
        score += (cpu_af - opp_af) * 105;
        score += (cpu_land - opp_land) * 45;
        break;
    }

    // Commander danger/pressure from attack cache.
    if (st.cmd_col[cpu_pl] >= 0) {
        int cpu_cmd_atk = st.atk.counts[opp_pl][st.cmd_row[cpu_pl]][st.cmd_col[cpu_pl]];
        int cpu_cmd_def = st.atk.counts[cpu_pl][st.cmd_row[cpu_pl]][st.cmd_col[cpu_pl]];
        score -= cpu_cmd_atk * 120;
        score += cpu_cmd_def * 30;
    }
    if (st.cmd_col[opp_pl] >= 0) {
        int opp_cmd_atk = st.atk.counts[cpu_pl][st.cmd_row[opp_pl]][st.cmd_col[opp_pl]];
        int opp_cmd_def = st.atk.counts[opp_pl][st.cmd_row[opp_pl]][st.cmd_col[opp_pl]];
        score += opp_cmd_atk * 120;
        score -= opp_cmd_def * 30;
    }

    // Mobility proxy from attacked/controlled squares.
    score += (st.atk.attacked_square_count[cpu_pl] - st.atk.attacked_square_count[opp_pl]) * 4;

    // Aircraft survivability in hostile AA coverage + carrier activity signal.
    int cpu_af_risk = 0, opp_af_risk = 0;
    int cpu_carried = 0, opp_carried = 0;
    for (const auto& p : st.pieces) {
        if (!on_board(p.col, p.row)) continue;
        if (p.carrier_id >= 0) {
            if (p.player == cpu_player) cpu_carried++;
            else                        opp_carried++;
        }
        if (p.kind == "Af" && !p.hero && in_aa_range(st.pieces, p.col, p.row, p.player)) {
            if (p.player == cpu_player) cpu_af_risk++;
            else                        opp_af_risk++;
        }
    }
    score -= cpu_af_risk * 140;
    score += opp_af_risk * 140;
    score += (cpu_carried - opp_carried) * 16;
...
static int board_score_cpu_impl(const PieceList& pieces, const std::string& perspective,
    ...
    MoveGenContext eval_ctx = build_movegen_context(pieces);
    ...
    for (auto& p : pieces) {
        ...
        if (p.kind != "H" && p.kind != "C" && !p.hero) {
            ...
            if (oc && cache) {
                ...
                MoveList<256> mvs;
                get_moves_with_ctx(p, eval_ctx, mvs);
                for (auto& m : mvs) {
                    if (m.dc == oc->col && m.dr == oc->row) {
    ...
    if (my_cmd) {
        ...
        int escapes = 0;
        MoveList<256> cmd_moves;
        get_moves_with_ctx(*my_cmd, eval_ctx, cmd_moves);
        for (auto& m : cmd_moves) {
            int opp_pl = (perspective == "red") ? 1 : 0;
            if (!cache || cache->counts[opp_pl][m.dr][m.dc] == 0) escapes++;
        }
...
static int quiesce(SearchState& st, int alpha, int beta,
    ...
    for (auto& p : st.pieces) {
        ...
        MoveList<256> mvs;
        get_moves_with_ctx(p, qctx, mvs);
        for (auto& m : mvs) {
            const Piece* t = piece_at_c(st.pieces, m.dc, m.dr);
            ...
            int sv = is_cap ? see(st.pieces, m.dc, m.dr, perspective) : 0;
            if (ncaps < 128) caps[ncaps++] = {p.id, m.dc, m.dr, sv, !is_cap};
        }
    }
...
// Server-safety: serialize top-level searches so TT/history globals are never
// mutated by multiple independent game requests at the same time.
static std::mutex g_search_serial_mutex;
...
static int alphabeta(SearchState& st, int depth, int alpha, int beta,
    ...
    std::string win = check_win(st.pieces, last_mover, st.mode);
    if (!win.empty()) {
    ...
    int raw_static_eval = st.quick_eval + quick_eval_context_terms(st, cpu_player);
    ...
    MoveList<256> moves;
    all_moves_for(st.pieces, st.turn, moves);
    if (moves.empty()) {
        build_attack_cache(st);
    ...
    QuietEntry searched_quiets[64];
    int searched_quiet_count = 0;

    for (auto& m : moves) {
        if (time_up()) break;
        int moved_idx0 = find_piece_idx_by_id(st, m.pid);
        ...
        if (is_quiet && moved_ki >= 0) {
            if (searched_quiet_count < (int)(sizeof(searched_quiets) / sizeof(searched_quiets[0])))
...
static AIResult mcts_ab_root_search(const PieceList& pieces,
    ...
    std::lock_guard<std::mutex> search_guard(g_search_serial_mutex);
    ...
    SearchState root_st = make_search_state(pieces, cpu_player, cpu_player, mode);
    MoveList<256> all_moves;
    all_moves_for(root_st.pieces, cpu_player, all_moves);
    ...
    auto select_path = [&](SelectionPath& sel) -> bool {
        ...
        if (l1.visits >= 2) {
            if (!l1.expanded) {
                l1.expanded = true;
                AllMoves l2_moves;
                all_moves_for(l1.state.pieces, opp_player, l2_moves);
...
static bool is_legal_book_move(const SearchState& st, const std::string& cpu_player, const MoveTriple& cand) {
    int idx = find_piece_idx_by_id(st, cand.pid);
...
static int opening_immediate_risk(const PieceList& pieces, const std::string& cpu_player) {
    AllMoves om;
    all_moves_for(pieces, opp(cpu_player), om);
...
static bool opening_book_pick(const SearchState& st, const std::string& cpu_player, MoveTriple& out) {
    ...
    for (auto& m : book) {
        ...
        if (has_immediate_winning_move(np, opp(cpu_player), st.mode)) continue;
...
static AIResult cpu_pick_move(const PieceList& pieces, const std::string& cpu_player,
    ...
    SearchState root = make_search_state(pieces, cpu_player, cpu_player, mode);
    MoveList<256> all_moves;
    all_moves_for(root.pieces, cpu_player, all_moves);
    if (all_moves.empty()) return {false, {}};
    ...
    for (int cur_depth = 1; cur_depth <= max_depth; cur_depth++) {
        ...
        while (!time_up()) {
            ...
            for (auto& m : root_moves) {
                if (time_up()) break;
                int moved_idx = find_piece_idx_by_id(root, m.pid);
                ...
                if (opening_phase) {
                    ...
                    opp_immediate_win = has_immediate_winning_move(root.pieces, opp(cpu_player), mode);
...
static AIResult smp_cpu_pick_move(const PieceList& pieces, const std::string& cpu_player,
    ...
    auto run_worker = [&](int thread_id) {
        ...
        g_deadline = shared.deadline;
        reset_time_state();
        smp_worker(thread_id, pieces, cpu_player, mode, max_depth, shared);
    };
...
struct Game {
    ...
    void finish_turn(const std::string& log, const MoveRecord& rec) {
        ...
        std::string wm = check_win(pieces, current, selected_mode);
        if (!wm.empty()) {
            state_history.push_back(pieces);
    ...
    void start_cpu_move() {
        ...
        GameMode mode = selected_mode;
        int depth = cpu_depth;
        double tlimit = cpu_time_limit;

        cpu_thread = std::thread([this, pieces_copy, cpu_pl, mode, depth, tlimit]() {
            try {
                reset_search_tables();
                ...
                if (g_use_mcts) {
                    ...
                    {
                        SearchState root_st = make_search_state(pieces_copy, cpu_pl, cpu_pl, mode);
                        MoveTriple book_mv{};
                    ...
                    res = mcts_ab_root_search(pieces_copy, cpu_pl,
                                             mode, MCTS_AB_DEPTH, mcts_time, &cpu_stop);
                    ...
                    if (res.found && !cpu_stop.load(std::memory_order_relaxed)) {
                        ...
                        AIResult verify = smp_cpu_pick_move(pieces_copy, cpu_pl,
                                                            depth, verify_time, mode, &cpu_stop);
                        // Prefer the deeper AB result if it finds a move
                        if (verify.found) res = verify;
                    }
                } else {
                    res = smp_cpu_pick_move(pieces_copy, cpu_pl, depth, tlimit, mode, &cpu_stop);
                }
...
static int run_headless_sim(const SimOptions& opt) {
    ...
    for (int g = 0; g < opt.games; g++) {
        ...
        std::string init_why;
        if (!validate_state_for_sim(pieces, opp(starter), GameMode::FULL_BATTLE, &init_why)) {
            std::cerr
        ...
        for (int ply = 0; ply < opt.max_plies; ply++) {
            ...
            AIResult r = cpu_pick_move(pieces, turn, opt.depth, time_limit_secs, GameMode::FULL_BATTLE);
            if (!r.found) {
                draws++;
                finished = true;
                break;
            ...
            std::string why;
            if (!validate_state_for_sim(pieces, turn, GameMode::FULL_BATTLE, &why)) {
                std::cerr
                    << "[sim] invalid state"
                    << " seed=" << opt.seed
                    << " game=" << g
            ...
            std::string win = check_win(pieces, turn, GameMode::FULL_BATTLE);
            if (!win.empty()) {
