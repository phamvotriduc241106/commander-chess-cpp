...
// === CHANGED ===
// Full-safety version: retains legality check for untrusted call sites (UI, tests).
static bool make_move_inplace_snapshot(SearchState& st, const MoveTriple& m,
    ...
    // XOR out all pieces that will change before applying the move
    uint64_t hash_delta = 0;
    // XOR out moved piece at old position
    hash_delta ^= zobrist_piece_key(st.pieces[moved_idx]);
    // XOR out captured piece (if any)
    if (u.had_capture) hash_delta ^= zobrist_piece_key(st.pieces[captured_idx]);
    // XOR out any passengers on the moved piece (carrier unload changes their state)
    for (auto& p : st.pieces) {
        if (p.carrier_id == m.pid) hash_delta ^= zobrist_piece_key(p);
    }

    if (!apply_move_unchecked_inplace(st.pieces, m.pid, m.dc, m.dr, st.turn)) return false;

    // XOR in the moved piece at new position (and any state changes: hero, carrier, etc.)
    int moved_idx_after = find_piece_idx_by_id_fast(st, m.pid);
    if (moved_idx_after >= 0) hash_delta ^= zobrist_piece_key(st.pieces[moved_idx_after]);
    // XOR in any passengers that were on the moved piece (their state may have changed)
    for (auto& p : st.pieces) {
        if (p.carrier_id == m.pid) hash_delta ^= zobrist_piece_key(p);
    }
    // Toggle turn
    hash_delta ^= g_ZobristTurn[st.turn == Player::Red ? 0 : 1];
    hash_delta ^= g_ZobristTurn[opp(st.turn) == Player::Red ? 0 : 1];
...
// Search-optimized version: skips the redundant legality check since all_moves_for()
// already guarantees that every move in the MovePicker is legal for the current position.
// Also uses incremental Zobrist hashing for O(1) hash updates instead of O(n) rehash.
static bool make_move_inplace_search(SearchState& st, const MoveTriple& m,
                                     Player cpu_player, UndoMove& u) {
    u = UndoMove{};
    if (!on_board(m.dc, m.dr)) return false;
    int moved_idx = find_piece_idx_by_id_fast(st, m.pid);
    ...
    // ** Finding 1: Skip redundant legality check — the move was already
    // generated by all_moves_for() and only reordered by MovePicker. **
    ...
    // ** Finding 2: Incremental Zobrist hashing — O(1) instead of O(n) **
    // XOR out pieces that will change before applying the move
    uint64_t hash_delta = 0;
    hash_delta ^= zobrist_piece_key(st.pieces[moved_idx]);
    if (u.had_capture) hash_delta ^= zobrist_piece_key(st.pieces[captured_idx]);
    for (auto& p : st.pieces) {
    ...
    // XOR in pieces at their new positions/states
    int moved_idx_after = find_piece_idx_by_id_fast(st, m.pid);
    if (moved_idx_after >= 0) hash_delta ^= zobrist_piece_key(st.pieces[moved_idx_after]);
    for (auto& p : st.pieces) {
        if (p.carrier_id == m.pid) hash_delta ^= zobrist_piece_key(p);
...
static uint64_t perft_impl(SearchState& st, int depth, Player cpu_player) {
    ...
    for (const auto& m : moves) {
        UndoMove u;
        if (!make_move_inplace_search(st, m, cpu_player, u)) continue;
...
static int quiesce(SearchState& st, int alpha, int beta,
    ...
    for (int ci = 0; ci < ncaps; ci++) {
        ...
        UndoMove u;
        if (!make_move_inplace_search(st, {c.pid, c.dc, c.dr}, cpu_player, u)) continue;
        ...
        int se_extension = 0;
        {
            ...
            if (is_hash_move && tte && tte->depth >= search_depth - 1 && search_depth >= 5 &&
                ...
                for (auto& om : moves) {
                    ...
                    UndoMove su;
                    if (!make_move_inplace_search(st, om, cpu_player, su)) continue;
