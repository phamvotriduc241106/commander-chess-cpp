...
static PieceList apply_move(const PieceList& pieces, int piece_id, int dc, int dr, const std::string& player) {
    ...
    if (target && target->player == player) return np;
...
static int find_piece_idx_at(const PieceList& pieces, int col, int row) {
    for (int i = 0; i < (int)pieces.size(); i++)
        if (pieces[i].col==col && pieces[i].row==row) return i;
    return -1;
}
...
static bool validate_state(const PieceList& pieces) {
    ...
    for (auto& p : pieces) {
        ...
        if (!occ.insert({p.col, p.row}).second) return false;
    }
    return true;
}
...
static bool validate_state_for_sim(const PieceList& pieces,
    ...
    for (auto& p : pieces) {
        ...
        if (!occ.insert({p.col, p.row}).second) {
...
static void build_attack_cache(SearchState& st) {
    ...
    for (auto& p : st.pieces) {
        ...
        for (auto& m : mvs) {
            if (on_board(m.first, m.second)) st.atk.counts[pl][m.second][m.first]++;
        }
    }
    st.atk.valid = true;
    st.atk.key = st.hash;
...
// Heap-allocated TT with configurable size
...
static void tt_resize(size_t size_mb) {
    ...
    g_TT = new TTCluster[g_tt_count]();
}

static void tt_ensure_allocated() {
    if (g_TT) return;
    for (size_t mb : {128, 64, 32, 8}) {
...
// Zobrist hashing (compact: only g_ZK is used)
...
static inline uint64_t zobrist_piece_key(const Piece& p) {
    ...
    int pl = p.player=="red" ? 0 : 1;
    int hi = p.hero ? 1 : 0;
    return g_ZK[ki][p.row][p.col][pl][hi];
}
...
static bool make_move_inplace(SearchState& st, const MoveTriple& m,
    ...
    if (target_idx >= 0) {
        ...
        navy_stays = (st.pieces[moved_idx].kind=="N" && !is_sea(m.dc,m.dr));
        u.had_capture = true;
    ...
    moved_idx = find_piece_idx_by_id(st.pieces, m.pid);
    if (moved_idx >= 0 && !st.pieces[moved_idx].hero) {
        const Piece* oc = nullptr;
        for (auto& p : st.pieces)
            if (p.player==opp(st.turn) && p.kind=="C") { oc=&p; break; }
        if (oc) {
            auto mvs = get_moves(st.pieces[moved_idx], st.pieces);
            for (auto& mv : mvs) {
                if (mv.first==oc->col && mv.second==oc->row) {
                    Piece np = st.pieces[moved_idx];
                    np.hero = true;
                    update_piece(moved_idx, np);
                    break;
...
// Board dimensions for history indexing: 11 kinds × 11 cols × 12 rows
static const int H_KINDS = 11, H_COLS = 11, H_ROWS = 12;

// ── Per-thread search data for Lazy SMP ──────────────────────────────────
struct ThreadData {
    MoveTriple killers[MAX_PLY][2];
    bool killers_set[MAX_PLY][2];
    // Flat history: [kind_index][col][row] — O(1) access, memset-clearable
    int history[H_KINDS][H_COLS][H_ROWS];
...
static int g_history[H_KINDS][H_COLS][H_ROWS];
...
static const TTEntry* tt_probe(uint64_t h) {
    ...
    for (int i = 0; i < TT_BUCKET; i++) {
        if (c.e[i].key != h) continue;
        if (!best || c.e[i].depth > best->depth) best = &c.e[i];
    }
    return best;
}
...
static void tt_store(uint64_t h, int depth, int flag, int val, MoveTriple best) {
    ...
    int repl = 0;
    int best_metric = -99999999;
    for (int i = 0; i < TT_BUCKET; i++) {
        if (c.e[i].key == h) { repl = i; best_metric = 99999999; break; }
        if (c.e[i].key == 0) { repl = i; best_metric = 99999999; break; }
        int age_pen = (int)(uint8_t)(g_tt_age - c.e[i].age);
        int metric = age_pen * 1024 - (int)c.e[i].depth;
        if (metric > best_metric) { best_metric = metric; repl = i; }
    }
    TTEntry& e = c.e[repl];
    e.key   = h;
...
static int td_history_score(const ThreadData& td, int ki, int dc, int dr) {
    if (ki<0||ki>=H_KINDS||dc<0||dc>=H_COLS||dr<0||dr>=H_ROWS) return 0;
    return td.history[ki][dc][dr];
}

static void td_update_history(ThreadData& td, int ki, int dc, int dr, int depth) {
    if (ki<0||ki>=H_KINDS||dc<0||dc>=H_COLS||dr<0||dr>=H_ROWS) return;
    td.history[ki][dc][dr] += depth*depth;
}

static void td_penalise_history(ThreadData& td, int ki, int dc, int dr, int depth) {
    if (ki<0||ki>=H_KINDS||dc<0||dc>=H_COLS||dr<0||dr>=H_ROWS) return;
    int& v = td.history[ki][dc][dr];
    v -= depth*depth;
...
static void penalise_history(int ki, int dc, int dr, int depth) {
    if (ki<0||ki>=H_KINDS||dc<0||dc>=H_COLS||dr<0||dr>=H_ROWS) return;
    int& v = g_history[ki][dc][dr];
    v -= depth*depth;
    if (v < -400000) v = -400000;
}
...
static AllMoves order_moves(const AllMoves& moves, const PieceList& pieces,
    ...
    for (auto& m : moves) {
        ...
        // 2. Captures — scored by SEE (winning captures first, losing last)
            ...
            int victim_val = PIECE_VALUE.count(target->kind) ? PIECE_VALUE[target->kind] : 0;
            int attacker_val = PIECE_VALUE.count(piece->kind) ? PIECE_VALUE[piece->kind] : 1;
        ...
        // 4. History heuristic for quiet moves
            ...
            score = td ? td_history_score(*td, ki, m.dc, m.dr)
                       : history_score(ki, m.dc, m.dr);
...
static int commander_attackers_cached(SearchState& st, const std::string& player) {
    int cc = -1, cr = -1;
    for (auto& p : st.pieces) {
        if (p.player==player && p.kind=="C") { cc = p.col; cr = p.row; break; }
    }
    if (cc < 0) return 0;
...
static int board_score(const PieceList& pieces, const std::string& perspective,
    ...
    int THREAT_BONUS       = 260;
    int SPACE_ADV_WEIGHT   = (phase > 128) ? 2 : 4;  // space matters more in endgame
    int SPACE_CENTER_BONUS = (phase > 128) ? 10 : 16;
    int CMD_ATTACK_WEIGHT  = (phase > 128) ? 90 : 70;
    int TEMPO_BONUS        = 18;
    int CONTEMPT_BONUS     = 12;
    ...
    for (auto& p : pieces) {
        ...
        // Material
        int mat = PIECE_VALUE.count(p.kind) ? PIECE_VALUE[p.kind] : 0;
        ...
        if (cache && p.kind != "C") {
            ...
            if (atk > 0 && def == 0) {
                // Undefended and attacked: penalty proportional to piece value
                hanging = -(mat / 4);
    ...
    if (opp_cmd) {
        ...
        score += (ring_att - ring_def) * 12;
        score -= ring_escape * 8; // fewer escapes for enemy = better
    }
    ...
    if (cache) {
        ...
        int opp_pl = 1 - my_pl;
        int my_squares = 0, opp_squares = 0;
        for (int r = 0; r < 12; r++) {
            for (int c = 0; c < 11; c++) {
                if (cache->counts[my_pl][r][c] > 0) my_squares++;
                if (cache->counts[opp_pl][r][c] > 0) opp_squares++;
            }
        }
    ...
    // ── Strategic objective pressure ─────────────────────────────────────
    score += (my_navy - opp_navy) * 1200;
    if (my_navy == 1)  score -= 900;
    if (opp_navy == 1) score += 900;
    if (my_navy == 0)  score -= 2000;
    if (opp_navy == 0) score += 2000;
...
static bool time_up() {
    ...
    if (++g_time_check_counter & 4095) return false;  // check every 4096 nodes
...
static int alphabeta(SearchState& st, int depth, int alpha, int beta,
    ...
    if (!hash_move_ptr && depth >= 4 && !pv_node) {
        depth--;
    }
    ...
    // ── Null Move Pruning (both sides + verification at high depth) ──────
    if (null_ok && depth >= 3 && !pv_node) {
        int stm_pieces = 0;
        for (auto& p : st.pieces) if (p.player==st.turn) stm_pieces++;
        if (stm_pieces > 4) {
            UndoMove nu;
            SearchState& ns = st;
            ...
            ns.hash ^= g_ZobristTurn[ns.turn=="red" ? 0 : 1];
            ns.atk.valid = false;
            // Adaptive reduction: R increases with depth
            int R = 2 + depth / 4 + (improving ? 0 : 1);
    ...
    int pre_my_navy = count_kind_for(st.pieces, cpu_player, "N");
    ...
    for (auto& m : moves) {
        ...
        // Singular extension test for TT-best move.
        bool singular_extend = false;
        if (is_hash_move && tte && tte->depth >= depth-1 && depth >= 5 && !time_up()) {
            int singular_beta = tte->val - 90;
            bool singular = true;
            int tested = 0;
            for (auto& om : moves) {
                if (same_move(om, m)) continue;
                if (tested >= 6 || time_up()) break;
                UndoMove su;
                if (!make_move_inplace(st, om, cpu_player, su)) continue;
                int sv = alphabeta(st, depth-2, singular_beta-1, singular_beta,
                                   false, cpu_player, ply+1, false, &om, td);
                unmake_move_inplace(st, su);
                tested++;
                if (sv >= singular_beta) { singular = false; break; }
            }
            singular_extend = singular;
        }
        ...
        int post_my_navy = count_kind_for(st.pieces, cpu_player, "N");
        int rule_ext = 0;
        ...
        if (move_index == 0) {
            ...
            // Full-window re-search if PVS fails in PV node
        ...
        if (node_is_max) {
            ...
            if (beta <= alpha) {
                if (!is_capture) {
                    ...
                    if (moved_ki >= 0) {
                        if (td) { td_update_history(*td, moved_ki, m.dc, m.dr, depth);
                                  td_update_cont_history(*td, prev_move, moved_ki, m.dc, m.dr, depth); }
                        else    { update_history(moved_ki, m.dc, m.dr, depth);
                    ...
                    for (auto& sq : searched_quiets) {
                        ...
                        if (td) td_penalise_history(*td, sq.ki, sq.dc, sq.dr, depth);
                        else    penalise_history(sq.ki, sq.dc, sq.dr, depth);
                    }
                    if (prev_move && on_board(prev_move->dc, prev_move->dr)) {
...
static bool opening_book_pick(const SearchState& st, const std::string& cpu_player, MoveTriple& out) {
    ...
    const int n1 = find_piece_idx_by_id(st.pieces, 19);
    const int n2 = find_piece_idx_by_id(st.pieces, 20);
    if (n1 >= 0 && n2 >= 0) {
        const Piece& a = st.pieces[n1];
        const Piece& b = st.pieces[n2];
        if (a.col==0 && a.row==10 && b.col==0 && b.row==8) {
            book.push_back({20,1,8});
            book.push_back({19,1,10});
        } else if (a.col==0 && a.row==10) {
            book.push_back({19,1,10});
        } else if (b.col==0 && b.row==8) {
            book.push_back({20,1,8});
        }
    }
    // Delay non-capture AF flights in the very first turns.
    if (!very_early_opening) {
        book.push_back({24,2,7});
        book.push_back({25,8,7});
        book.push_back({24,3,8});
        book.push_back({25,7,8});
    }
    book.push_back({37,5,6});
    book.push_back({29,4,7});
    book.push_back({30,6,7});
...
static AIResult cpu_pick_move(const PieceList& pieces, const std::string& cpu_player,
    ...
    for (int cur_depth = 1; cur_depth <= max_depth; cur_depth++) {
        ...
        // ── Aspiration Windows (tighter initial delta) ────────────────────
        ...
        while (!time_up()) {
            ...
            for (auto& m : root_moves) {
                ...
                int val = alphabeta(root, cur_depth-1, window_alpha, window_beta, false,
...
static int smp_thread_count() {
    ...
    if (hw <= 0) hw = 1;
    // Use all cores but cap at 16 for diminishing returns
    return std::min(std::max(hw, 1), 16);
}
...
static void smp_worker(int thread_id, const PieceList& pieces,
    ...
    for (int cur_depth = start_depth; cur_depth <= max_depth; cur_depth++) {
        ...
        if (completed || cur_best_val > -900000) best = cur_best;
...
static AIResult smp_cpu_pick_move(const PieceList& pieces, const std::string& cpu_player,
    ...
    if (time_limit_secs < 3.0) num_threads = std::min(num_threads, 4);
    if (max_depth <= 4) num_threads = std::min(num_threads, 2);
...
struct Game {
    ...
    void set_difficulty(int d) {
        ...
        else { cpu_depth=8; cpu_time_limit=16.0; }
    }
        ...
        if (is_capture) {
            pieces.erase(
                std::remove_if(pieces.begin(), pieces.end(),
                    [captured_id](const Piece& p) { return p.id == captured_id; }),
                pieces.end()
            );
            log += " x" + captured_kind;
    ...
    void start_cpu_move() {
        ...
        cpu_thread = std::thread([this, pieces_copy, cpu_pl, depth, tlimit]() {
            try {
                reset_search_tables();
                AIResult res = smp_cpu_pick_move(pieces_copy, cpu_pl, depth, tlimit, &cpu_stop);
...
static void print_usage(const char* prog) {
    std::cout
        << "Usage:\n"
        << "  " << prog << "\n"
        << "  " << prog << " --sim [--games N] [--seed S] [--depth D] [--time_ms T] [--max_plies P] [--start MODE]\n"
        << "\n"
